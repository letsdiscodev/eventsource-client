...
var node_stream = require("node:stream"), eventsourceParser = require("eventsource-parser");
...
function createEventSource$1(optionsOrUrl, { getStream: getStream2 }) {
  ...
  } = options, { fetch, url, initialLastEventId } = validate(options), requestHeaders = { ...options.headers }, onCloseSubscribers = [], subscribers = onMessage ? [onMessage] : [], emit = (event) => subscribers.forEach((fn) => fn(event)), parser = eventsourceParser.createParser({ onEvent, onRetry });
  ...
  function connect() {
    request || (readyState = CONNECTING, controller = new AbortController(), request = fetch(url, getRequestOptions()).then(onFetchResponse).catch((err) => {
      request = null, !(err.name === "AbortError" || err.type === "aborted") && (onConnectionError(err), scheduleReconnect());
    }));
  }
  function close() {
  ...
  function scheduleReconnect() {
    onScheduleReconnect({ delay: reconnectMs }), readyState = CONNECTING, reconnectTimer = setTimeout(connect, reconnectMs);
  }
